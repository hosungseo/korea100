...
- 제도 109개 모두 표준 캔버스와 상태 인식형 swimlane 업무구조도를 제공합니다.
- 각 제도는 법적 근거, 행위자, 절차, 산출 문서, 기한, 병목, 개선점과 현장 검증 항목을 포함합니다.
- 웹앱은 Next.js 정적 내보내기로 109개 제도 상세 페이지를 생성합니다.
...
- `/verification/`에서 577개 현장 검증 항목을 분야와 검증 영역별로 공개합니다.
- 법령 기준일은 각 JSON의 `asOfDate`에 기록합니다.
- 109개 제도에 공식 출처 475건을 연결했습니다. 고유 법적 근거 357건은 국가법령정보센터 식별자와 연결되어 있습니다.
- 캔버스와 1,837개 절차 노드의 명시 조문 인용 4,340건을 현행 원문과 대조했고, 4,340건 모두 조문 번호의 존재를 확인했습니다.
- 82개 제도는 `article-verified`이고, 26개는 지역·기관·내부규정·부처 문서 범위를 지정해야 하는 출처 30건을 사유별로 기록했으며, 1개는 원문 연결 단계입니다.
...
- 577개 현장 검증 큐의 ID, 제도 참조, 필수 필드와 집계
...
- `docs/field-verification-queue.json`: 577개 현장 검증 작업 큐
