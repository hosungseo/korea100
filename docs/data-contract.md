...
- `web/data/institutions/*.json`: 상세 페이지와 빌드 시 파생 데이터의 원본 100개
...
- `/` 홈: 경량 `InstitutionSummary` 100개를 먼저 렌더링하고, 상세 검색·비교 데이터는 사용 시에만 정적 JSON으로 불러온다.
...
- `/verification`: 448개 `fieldVerification` 항목을 분야·검증 영역별로 검색하고 공개 근거 제보로 연결한다.
