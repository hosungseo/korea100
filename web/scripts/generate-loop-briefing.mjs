...
if (!process.argv.includes("--no-judge")) {
  try {
    const prompt =
      ...
      "형식(한 장짜리 보고서에 들어가야 하므로 전체 800자 이내 — 넘기지 말 것):\n" +
      `📡 광주 반도체·군공항 일일 동향 (${loop.generatedAt})\n` +
      "\n■ 주요 보도내용 — 오늘 보도된 사실 3건. 각 항목: 제목 줄 + 보도 내용 1문장(32자 이내 — 한 줄에 들어가야 한다).\n" +
      "  여기에는 '보도된 것'만 쓴다. 우리 판정(상태 전환 검토·분류·대상 지정)과 요구사항은 " +
      "절대 쓰지 말고 아래 절차·조치 항목으로 보낸다. 문장은 '보도됨/전해짐/밝힘' 전언 형식으로 끝낸다.\n" +
      "\n■ 분야별 절차 진행상황 — 5개 분야(군공항/산단·인허가/전력/용수/건축·가동) 각 1줄. 관문 ID를 반드시 괄호로 병기하고(표의 별도 칸으로 뽑아 쓴다), ID를 뺀 서술은 26자 이내.\n" +
      "  그 보도가 어느 관문(ID 병기)의 어느 절차에 걸리는지, 상태 전환 검토 대상인지를 쓴다. " +
      "여기가 우리 판정이다.\n" +
      "\n■ 리스크·갈등 — 3건, 각 1줄(36자 이내)\n" +
      "\n■ 조치 필요사항 — 결정·확인이 필요한 것 2줄(36자 이내). 그 다음 줄은 불릿(–) 없이 ※ 로 시작해 " +
      "판 수치(관문·절차·기관·상태 전환 검토·신규 후보) 1줄\n" +
      `\n마지막 줄: 상세: ${URL}\n` +
      "본문만 출력. 인사말·설명·코드블록 금지 — 첫 글자가 곧 📡 로 시작하는 제목 줄이어야 한다. " +
      "마크다운 강조 기호(**, __, #, 백틱)도 절대 쓰지 말 것 — " +
      "평문 텔레그램 메시지이자 한글 보고서 원문으로 그대로 쓰인다.\n\n데이터:\n" + JSON.stringify(payload);
    const out = execFileSync("claude", ["-p", prompt], { encoding: "utf8", timeout: 240_000 }).trim();
    // 모델이 "I'll write the briefing…" 같은 서두를 붙일 때가 있다 — 제목 줄부터 자른다
    const head = out.indexOf("📡");
    const cleaned = (head > 0 ? out.slice(head) : out).trim();
    if (cleaned.length > 200 && cleaned.includes("주요 보도내용")) text = cleaned.slice(0, 3900);
