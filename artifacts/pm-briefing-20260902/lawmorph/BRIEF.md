...
## 구성 (345.2초 · 3부 23챕터, 2026-08-29 5차)
...
| mont | 10.0s | 환경영향평가에 이어 — 공장설립승인(제13조, 20일 기한) 원문 카드→구조도 (소상공인 정책자금은 5차에서 삭제) |
| reg | 9.0s | 제도 타일 그리드 + 카운터(K100.instCount, 현재 647) |
| auto | 8.6s | 자동화된 증가 — 요청 메일→AI 에이전트 초안→사람 검증→등재 4스텝 |
| part2 | 9.6s | 전체 3부 구성 로드맵 — **2부(미시) 하이라이트** |
| **— 2부 : 미시로, 실증으로 —** | | |
...
| close | 12.6s | 실전 AX 가이드라인으로 — 현재 연구 중 + "여기까지가 2부" |
| **— 3부 : 거시로, 문샷으로 —** | | |
| p3 | 10.0s | 3부 시작 — "2부는 이미 하고 있는 일 / 3부는 지금은 못 하는 일부터" |
...
| **mdemo** | 13.0s | **5극3특 지도 실사이트 녹화(assets/fivepoles-demo.mp4, 12.2s) — 총리 브리핑·관문 클릭** |
| **board** | 12.0s | **축을 뒤집으면 부처 상황판(설명) — 18부처·관문 135·소관 미특정 58 + 최대 레버리지** |
| **bdemo** | 21.0s | **부처 상황판 실사이트 녹화(assets/ministry-demo.mp4, 19.9s) — 레버리지 정렬·부처 상세·사업 필터** |
| **fin** | 13.0s | **최종 클로징 — 제도 647 / 프로젝트 4 / 관문 135 / 부처 18 + 로고 락업** |
