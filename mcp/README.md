...
전체 653개를 자동 변환하지 않는다. 다음 열일곱 제도만 R2(`next-action`) 검증을 통과해 공개한다.
...
- 공업용수도 사업인가·실시계획·준공 (2026-09-01 승격)
- 지하수 개발·이용허가·영향조사·준공 (2026-09-01 승격)
- 공공폐수처리시설 기본계획·설계·준공·운영 (2026-09-01 승격)
- 공업용수·취수 허가 (2026-09-01 승격)
- 국가 도로·철도 SOC 추진 (2026-09-01 승격)
...
| `samples/gwangju-semiconductor-cluster.case.json` | 광주 반도체 클러스터 (제도 108종 중 13종 R2) | 부분 | N02·N03·N19·N20·N23 |
...
법제처 `lawService`의 ID 조회는 공포됐지만 시행일이 남은 개정본을 돌려줄 수 있다. 그런 판을
현행이라고 부르면 대조 결과 전체가 사실과 달라진다. 그래서 대조는 `target=eflaw` 검색으로
대조일에 시행 중이던 판의 MST와 시행일을 먼저 찾고, 그 시행일을 지정해 본문을 가져온다.
그 경로가 실패해 ID 조회로 떨어졌는데 돌아온 판이 시행 전이면 대조를 실패로 처리한다.
