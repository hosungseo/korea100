...
전체 653개를 자동 변환하지 않는다. 다음 열네 제도만 R2(`next-action`) 검증을 통과해 공개한다.
...
법제처 ID 조회는 공포됐지만 시행일이 남은 개정본을 돌려줄 수 있다. 그런 판을 현행이라고 부르면
대조 결과 전체가 사실과 달라지므로, 돌려받은 판의 시행일이 대조일보다 뒤면 대조를 실패로 처리한다.
